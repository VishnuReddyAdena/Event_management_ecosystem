...
import { Landmark, Eye, EyeOff, ArrowLeft, Layers, ShieldCheck } from 'lucide-react';
...
export default function OrganizerLogin() {
  ...
  // Base fields
  const [email, setEmail] = useState('organizer@platform.com');
  ...
  const [hasDeanAuth, setHasDeanAuth] = useState(false);
  const [authorizedDeanName, setAuthorizedDeanName] = useState('');
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'login') {
      ...
      if (!designation.trim()) {
        setError('Designation/Role within the organization is required.');
        return;
      }

      if (!hasDeanAuth) {
        setError('You must certify Dean/Administrator authorization to register as an Organizer.');
        return;
      }

      if (!authorizedDeanName.trim()) {
        setError('Dean/Administrator name is required for verification checks.');
        return;
      }
      ...
      try {
        ...
        const hash = await calculateSHA256(email, 'organizer', timestamp);
        await new Promise(resolve => setTimeout(resolve, 500));
        ...
        const profileData = {
          ...
          deanAuthorization: {
            certified: true,
            verifierName: authorizedDeanName
          },
          bio: `Dean/Admin Authorized Organizer representing ${orgName}.`,
          phone: '+1 555-0199'
        };
        ...
        <div style={{ padding: '40px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '24px', backdropFilter: 'blur(24px)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '32px', gap: '14px' }}>
            <div style={{ padding: '14px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px' }}>
              <Landmark size={28} style={{ color: '#ffffff' }} />
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '4px' }}>ORGANIZER COCKPIT</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '8px 14px', background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.12)', borderRadius: '10px', marginBottom: '24px' }}>
            ...
            <span style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.35)' }}>Access locked to <strong style={{ color: 'rgba(255,255,255,0.7)' }}>Organizer</strong> role only</span>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div>
                    ...
                    <input className="input-glass-premium" type="text" placeholder="Sarah" value={firstName} onChange={e => setFirstName(e.target.value)} required />
                  </div>
                  <div>
                    ...
                    <input className="input-glass-premium" type="text" placeholder="Jenkins" value={lastName} onChange={e => setLastName(e.target.value)} required />
                  </div>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.74rem', color: 'rgba(255,255,255,0.45)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>ORGANIZATION NAME</label>
                  <input className="input-glass-premium" type="text" placeholder="College of Computing" value={orgName} onChange={e => setOrgName(e.target.value)} required />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.74rem', color: 'rgba(255,255,255,0.45)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>DESIGNATION</label>
                  <input className="input-glass-premium" type="text" placeholder="Dean / Student Body President" value={designation} onChange={e => setDesignation(e.target.value)} required />
                </div>
                
                {/* Dean authorization verification */}
                <div style={{
                  ...
                  <label style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', cursor: 'pointer', userSelect: 'none' }}>
                    <input
                      type="checkbox"
                      checked={hasDeanAuth}
                      onChange={e => setHasDeanAuth(e.target.checked)}
                      style={{ marginTop: '3px', cursor: 'pointer' }}
                    />
                    <span style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.6)', lineHeight: '1.4' }}>
                      I certify that I hold official authorization from the Dean/Administration of the University.
                    </span>
                  </label>
                  {hasDeanAuth && (
                    <div>
                      <label style={{ display: 'block', fontSize: '0.68rem', color: 'rgba(255,255,255,0.4)', marginBottom: '4px', textTransform: 'uppercase' }}>DEAN / ADMINISTRATOR NAME</label>
                      <input className="input-glass-premium" type="text" placeholder="Dr. Robert Vance" value={authorizedDeanName} onChange={e => setAuthorizedDeanName(e.target.value)} required style={{ padding: '8px 12px !important', fontSize: '0.85rem !important' }} />
                    </div>
                  )}
                </div>
              </>
            ...
            <div>
              ...
              <input className="input-glass-premium" type="email" placeholder="organizer@platform.com" value={email} onChange={e => setEmail(e.target.value)} required />
            </div>
            ...
            <button type="submit" className="btn-solid" disabled={loading} style={{ width: '100%', marginTop: '6px', padding: '13px' }}>
              {loading ? 'Processing Securely...' : (mode === 'login' ? 'Access Organizer Cockpit' : 'Sign Up')}
            </button>
          </form>
          ...
          <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.05)', textAlign: 'center', fontSize: '0.74rem', color: 'rgba(255,255,255,0.2)' }}>Demo: organizer@platform.com / volunteer123</div>
        </div>
      </div>
    </div>
  );
