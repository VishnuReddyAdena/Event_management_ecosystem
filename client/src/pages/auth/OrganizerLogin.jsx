...
import { Landmark, Eye, EyeOff, ArrowLeft, Layers } from 'lucide-react';
...
export default function OrganizerLogin() {
  ...
  React.useEffect(() => {
    if (user && user.role === 'organizer') navigate('/dashboard/organizer');
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'login') {
        ...
        const u = await signup(name, email, password, 'organizer');
        navigate('/dashboard/organizer');
      }
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', position: 'relative', zIndex: 1 }}>
      <div style={{ width: '100%', maxWidth: '440px' }}>
        <Link to="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', color: 'rgba(255,255,255,0.35)', fontSize: '0.82rem', textDecoration: 'none', marginBottom: '32px', transition: 'color 0.2s' }}
        ...
        <div style={{ padding: '40px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '24px', backdropFilter: 'blur(24px)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '8px 14px', background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.12)', borderRadius: '10px', marginBottom: '24px' }}>
            ...
            <span style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.06em' }}>Access locked to <strong style={{ color: 'rgba(255,255,255,0.7)' }}>Organizer</strong> role only</span>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {mode === 'register' && (
              <div>
                ...
                <input className="input-glass" type="text" placeholder="Dr. Sarah Jenkins" value={name} onChange={e => setName(e.target.value)} required />
              </div>
            )}
            <div>
              ...
              <input className="input-glass" type="email" placeholder="organizer@platform.com" value={email} onChange={e => setEmail(e.target.value)} required />
            </div>
            ...
            {error && (
              <div style={{ padding: '10px 14px', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '8px', fontSize: '0.82rem', color: 'rgba(255,255,255,0.45)', background: 'rgba(255,255,255,0.02)' }}>
                ✗ {error}
              </div>
            )}
            <button type="submit" className="btn-solid" disabled={loading} style={{ width: '100%', marginTop: '6px', padding: '13px' }}>
              {loading ? 'Authenticating...' : (mode === 'login' ? 'Access Organizer Cockpit' : 'Create Organizer Account')}
            </button>
          </form>
          ...
          <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.05)', textAlign: 'center', fontSize: '0.74rem', color: 'rgba(255,255,255,0.2)' }}>
            Demo: organizer@platform.com / volunteer123
          </div>
        </div>
      </div>
    </div>
